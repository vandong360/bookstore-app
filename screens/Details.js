...
const Details = ({route, navigation}) => {
 ...
 return (
    ...
    <TouchableOpacity style={{marginTop:10, backgroundColor:'#fff'}} onPress={() => navigation.goBack()}>
        <Text><Ionicons name="arrow-back" size={20} color="black"></Ionicons></Text>
        </TouchableOpacity>
        <View style={{alignItems:'center', width:'100%', height:400, backgroundColor:'#fff'}}>
        <Text style={{marginTop:10, marginHorizontal: 10, fontWeight: 'bold', fontSize: 18, color:'#3e4242' }}>{book.name}</Text>
            <Image style = {styles.Image} source={book.image ? {uri: book.image } : null} />
        </View>
        <Text style={{marginTop:10, marginHorizontal: 10, fontWeight: 'bold', fontSize: 14, color:'gray' }}>{book.author}</Text>
        <Text style={{marginTop:10, marginHorizontal: 10, fontWeight: 'bold', fontSize: 18, color:'#ebb859' }}>{book.price} đ</Text>
        ...
        <View style={{
            ...
            <View style={{flexDirection:"column", flex:1}}>
                ...
                <Text style={{textAlign:'center', fontWeight: 'bold', fontSize: 14, color:'#3e4242'}}>{book.soTrang}</Text>
            </View>    
            <View style={{flexDirection:"column",flex:1}}>
                <Text style={{textAlign:'center', fontWeight: 'bold', fontSize: 14, color:'#ebb859'}}>Đánh giá</Text>
                <Text style={{textAlign:'center', fontWeight: 'bold', fontSize: 14, color:'#3e4242'}}>{book.rating}</Text>
            </View>
            <View style={{flexDirection:"column", flex:1}}>
                <Text style={{textAlign:'center', fontWeight: 'bold', fontSize: 14, color:'#ebb859'}}>Xuất Bản</Text>
                <Text style={{textAlign:'center', fontWeight: 'bold', fontSize: 14, color:'#3e4242'}}>{book.namXB}</Text>
            </View>
